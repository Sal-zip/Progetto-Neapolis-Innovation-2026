...
- Tema DaisyUI personalizzato **"rescue"**:

  | Colore | Uso |
  |---|---|
  | Blu `#2563EB` | Azioni principali |
  | Rosso `#DC2626` | SOS ed errori |
  | Ambra `#D97706` | Avvisi |
  | Verde `#059669` | Online |
  | Blu notte `#0F172A` | Barra laterale |

- Stile **"liquid glass"**: pannelli di vetro traslucido con sfocatura, bordi luminosi, angoli
  molto arrotondati e ombre morbide su uno sfondo sfumato.
- **Allarmi sempre opachi**: banner SOS, avvisi e mappa non usano la trasparenza, per restare
  leggibili in emergenza.
- **Logo**: giacca ad alta visibilità con bande catarifrangenti, disegnata in SVG nel codice.
- **Movimento ridotto**: l'animazione di entrata delle pagine e lo scorrimento dei marker si
  disattivano se il sistema operativo chiede di ridurre il movimento.

---

## 10. Scelte progettuali

- **Niente framework**: una dashboard con poche pagine e una connessione in tempo reale persistente
  non ne ha bisogno. Meno dipendenze, meno cose da imparare, nessun problema di rendering lato
  server.
- **Pagina singola invece di più pagine**: ricaricare la pagina chiuderebbe la connessione in tempo
  reale e si rischierebbe di perdere un SOS proprio durante la navigazione.
- **Nessuna View Transitions API**: durante la transizione il browser manda tutti i click a
  `<html>` e un click subito dopo il cambio pagina andava perso. Si usa una semplice animazione CSS
  di entrata.
- **Un solo codice sorgente** per la versione simulata e quella reale: cambia solo una variabile
  al momento della build.

---

## 11. Limiti noti e sviluppi futuri

Funzioni del documento dei requisiti (Maker Faire 2026) che la dashboard **non** copre: vanno
aggiunte qui se un giorno il gilet inizia a inviarle.

- **Segnalazioni dai gesti** oltre alla zona sicura: zona esplorata, pericolo trovato, traccia utile.
- **Conferma dell'applicazione verso il gilet** (ACK), diversa dalla presa in carico: serve al gilet
  per smettere di ritrasmettere un SOS.

Altro:

- La **mappa è stradale**: per l'uso in montagna servirebbe uno sfondo topografico.
- Il **tracciato PPG** nella pagina Operatori è illustrativo, non derivato dai dati.
- Il **messaggio a testo libero** diventerà utile con l'app Android per l'operatore, prevista come
  sviluppo futuro (stesso codice, impacchettato con Capacitor).
- La **causa dell'SOS** e lo **stato dei sensori** vengono dagli eventi SOS e STATUS ricevuti: finché
  un gilet non ne invia uno, la dashboard mostra "causa non indicata" o "stato dei sensori non
  ancora ricevuto".
